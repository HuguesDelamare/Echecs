...
class DatabaseModel:
    ...
    def playerInsertDB(self, serialized_player):
        TinyDB("./database/tournament_database.json").\
            table(self.database_table).insert(serialized_player)
    ...
    def select_tournament_player_by_id(self, selected_player):
        result = TinyDB("./database/tournament_database.json").\
            table('playerTable').\
            get(doc_id=selected_player)
        return result
    ...
    def get_all_tournament_players(self):
        try:
            list_of_players = []
            player_database = TinyDB("./database/tournament_database.json").\
                table('playerTable')
            if len(player_database) >= 8:
                ...
                print(str(len(player_database)) +
                      "players registred,"
                      " need at least 8 to start a tournament.")
        except TypeError:
            print('error')

    def get_all_tournament(self):
        tournament_database = TinyDB("./database/tournament_database.json")\
    ...
    def get_tournament_turn_count(self, tournament):
        tournament_database = \
            TinyDB("./database/tournament_database.json").\
            table('TournamentTable')
        tournament = tournament_database.\
            search(where('tournament_name') ==
                   tournament['tournament_name'])[0]
    ...
    def get_last_played_turn_of_tournament(self, tournament_name):
        tournament_database = TinyDB("./database/tournament_database.json").\
            table('TournamentTable')
        tournament = tournament_database.\
            search(where('tournament_name') == tournament_name)
    ...
    def end_tournament(self, tournament_name):
        tournament_database = TinyDB("./database/tournament_database.json").\
            table('TournamentTable')
        tournament_database.update(set('tournament_ongoing', False),
                                   (where('tournament_name') ==
                                    tournament_name))
        tournament_database.update(set('tournament_date_end',
                                       str(datetime.now().
                                           strftime("%d/%m/%Y %H:%M"))),
                                   (where('tournament_name') ==
                                    tournament_name))
    ...
    def get_all_turns_from_tournament(self, tournament_id):
        ...
        tournament_turns = \
            tournament_database.\
            get(doc_id=tournament_id)['tournament_turns']
    ...
    def get_tournament_players_from_tournament(self, tournament_id):
        ...
        tournament_players_list = \
            tournament_database.\
            get(doc_id=tournament_id)['tournament_players_list']
